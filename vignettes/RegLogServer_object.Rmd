...
Type of the message received from mailConnectors with response about confirmation
...
>modalDialog indicating successful registration is shown after receiving
this message to encourage observing the same change with any custom logic. After
successful update of database the appearance time of `register` message can be 
very brief, so it is better to write logic to observe this message `register_mail`
message.
...
- **logcontent**
  - username for which the password were changed.